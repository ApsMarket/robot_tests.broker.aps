...
Get Field Signed Date
    [Arguments]    ${id}
    ${signedDate}=    Get Text    ${id}
    ${signedDate}=    Replace String    ${signedDate}    ${SPACE}    T
    ${tz}=    Get Local TZ
    ${signedDate}=    Set Variable    ${signedDate}.000000+0${tz}:00
    Return From Keyword    ${signedDate}
